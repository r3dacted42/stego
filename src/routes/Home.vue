<template>
    ...
    <p>steganography is the art and science of hiding a message, file, or other data within another message,
        file, or object, without anyone else knowing that the hidden data exists. unlike cryptography, which
        just scrambles a message, steganography conceals its very existence.</p>
    <p>this project demonstrates two of the most common methods used on the web. use the nav links above to try them
        out!</p>

    <hr>

    <h3>image steganography (LSB)</h3>
    <p>this method hides data inside the pixels of an image. every pixel on your screen is made of Red, Green, and Blue
        (RGB) color channels. each channel has a value from 0 to 255.</p>
    <p>the Least Significant Bit (LSB) technique changes the very last bit of each channel's value. for example, a Red
    ...
    <p>this tool stores 3 bits in every pixel (one for Red, Green, and Blue), allowing us to hide surprisingly large
        messages or even entire files inside an ordinary-looking picture.</p>

    <h3>text steganography (zero-width Unicode)</h3>
    <p>this method hides data by inserting invisible characters between the visible letters of a text. Unicode, the
        standard for text on computers, includes "zero-width" characters that take up no space and are not rendered.</p>
    <p>we use four different zero-width characters to represent 2 bits of data at a time (e.g., <code>U+200B</code> =
    ...
    <p>while powerful, these techniques are very fragile and are intended for educational purposes, not for secure
        communication. keep these limitations in mind:</p>
    <ul>
